...
function PPTDownloader({
  ...
  */
   ...
   */
  const collectGroupElements = (group) => {
    if (!group) return [];
    const elements = [];
   ...
   */
  const applyGroupLayoutAdjustments = (groups, slides) => {
    ...
    groupedBySlide.forEach((slideGroups, slideIndex) => {
      ...
      const slideWidth = slide?.width ?? pptWidth;
      const slideHeight = slide?.height ?? pptHeight;
      ...
      for (const chart of group.charts) {
        ...
        try {
          ...
          if (chartMeta.chartType === "multibar") {
            ...
            chartOptions.barGrouping = chartMeta.barGrouping;
            chartOptions.catAxisLabelRotate = chartMeta.catAxisLabelRotate;
            chartOptions.barGapWidthPct = chartMeta.barGapWidthPct;
            chartOptions.barOverlapPct = chartMeta.barOverlapPct;
          }
          ...
          else if (chartMeta.chartType === "bar" && chartMeta.colors?.length > 0) {
